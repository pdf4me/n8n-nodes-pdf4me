...
export const description: INodeProperties[] = [
	...
	},
	{
		...
		hint: 'Extract structured data from invoices using AI technology for automated data entry. See our <b><a href="https://docs.pdf4me.com/integration/n8n/pdf4me-ai/ai-invoice-parser/" target="_blank">complete guide</a></b> for detailed instructions and examples.',
	},
	{
...
export async function execute(this: IExecuteFunctions, index: number) {
	...
	try {
		...
		if (error.code === 'ECONNRESET') {
						...
						throw new NodeOperationError(this.getNode(), `PDF4Me server error (500): ${error.message || 'The service was not able to process your request.'} | Debug: docLength=${docContent?.length}, docName=${docName}`, { itemIndex: index });
						...
						throw new NodeOperationError(this.getNode(), `PDF4Me API error (${error.statusCode}): ${error.message || 'Unknown error'} | Debug: docLength=${docContent?.length}, docName=${docName}`, { itemIndex: index });
		} else {
						throw new NodeOperationError(this.getNode(), `Connection error: ${error.message || 'Unknown connection issue'} | Debug: docLength=${docContent?.length}, docName=${docName}, errorCode=${error.code}`, { itemIndex: index });
		}
	}
		...
		try {
			if (typeof result === 'string') {
				...
				processedData = result;
						...
						throw new NodeOperationError(this.getNode(), `Failed to parse API response: ${error.message}`, { itemIndex: index });
		}

...
export async function execute(this: IExecuteFunctions, index: number) {
	...
	if (operation === ActionConstants.OptimizePdf) {
		inputDataType = this.getNodeParameter('createPdfAInputDataType', index) as string;
		outputFileName = this.getNodeParameter('createPdfAOutputFileName', index) as string;
		docName = this.getNodeParameter('createPdfADocName', index) as string;
		compliance = this.getNodeParameter('createPdfACompliance', index) as string;
		advancedOptions = this.getNodeParameter('createPdfAAdvancedOptions', index) as IDataObject;
	} else {
		...
		compliance = this.getNodeParameter('compliance', index) as string;
	...
	if (inputDataType === 'binaryData') {
		const binaryPropertyName = operation === ActionConstants.OptimizePdf
			? this.getNodeParameter('createPdfABinaryPropertyName', index) as string
		...
		docContent = operation === ActionConstants.OptimizePdf
			? this.getNodeParameter('createPdfABase64Content', index) as string
		...
		const pdfUrl = operation === ActionConstants.OptimizePdf
			? this.getNodeParameter('createPdfAUrl', index) as string
		...
		const filePath = operation === ActionConstants.OptimizePdf
			? this.getNodeParameter('createPdfAFilePath', index) as string

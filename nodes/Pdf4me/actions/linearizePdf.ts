...
export async function execute(this: IExecuteFunctions, index: number) {
	...
	let docName: string;
	let optimizeProfile: string;
	let advancedOptions: IDataObject;

	if (operation === ActionConstants.OptimizePdf) {
		inputDataType = this.getNodeParameter('linearizePdfInputDataType', index) as string;
		outputFileName = this.getNodeParameter('linearizePdfOutputFileName', index) as string;
		docName = this.getNodeParameter('linearizePdfDocName', index) as string;
		optimizeProfile = this.getNodeParameter('linearizePdfOptimizeProfile', index) as string;
		advancedOptions = this.getNodeParameter('linearizePdfAdvancedOptions', index) as IDataObject;
	} else {
		...
		optimizeProfile = this.getNodeParameter('optimizeProfile', index) as string;
	...
	if (inputDataType === 'binaryData') {
		const binaryPropertyName = operation === ActionConstants.OptimizePdf
			? this.getNodeParameter('linearizePdfBinaryPropertyName', index) as string
		...
		docContent = operation === ActionConstants.OptimizePdf
			? this.getNodeParameter('linearizePdfBase64Content', index) as string
		...
		const pdfUrl = operation === ActionConstants.OptimizePdf
			? this.getNodeParameter('linearizePdfUrl', index) as string
		...
		const filePath = operation === ActionConstants.OptimizePdf
			? this.getNodeParameter('linearizePdfFilePath', index) as string
	...
	const responseData = await pdf4meAsyncRequest.call(this, '/api/v2/LinearizePdf', body);

	if (responseData) {
		let fileName = outputFileName;
		if (!fileName || fileName.trim() === '') {
			const baseName = originalFileName ? originalFileName.replace(/\.[^.]*$/, '') : 'linearized_pdf_output';
		...
		if (!pdfBuffer || pdfBuffer.length < 100) {
			throw new Error('Invalid linearized PDF response from API. The converted file appears to be too small or corrupted.');
		}
		...
		return [
			{
				json: {
					...
					optimizeProfile,
					originalFileName,
					message: 'PDF linearized successfully - optimized for web viewing and faster loading',
				},
				binary: {
					data: binaryData,
				},

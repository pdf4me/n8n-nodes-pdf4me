...
export const description: INodeProperties[] = [
	{
		...
		displayOptions: {
			show: {
				operation: [ActionConstants.HtmlToPdf],
			},
		},
		options: [
			{
...
export async function execute(this: IExecuteFunctions, index: number) {
	...
	if (inputDataType === 'binaryData') {
		...
		console.log(`Downloading HTML from URL: ${htmlUrl}`);
		docContent = await downloadHtmlFromUrl(htmlUrl);
		console.log('HTML file successfully downloaded and encoded to base64');
...
async function downloadHtmlFromUrl(htmlUrl: string): Promise<string> {
	...
	try {
		...
		console.log(`HTML downloaded successfully: ${buffer.length} bytes`);

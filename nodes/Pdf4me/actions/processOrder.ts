...
export const description: INodeProperties[] = [
	{
		...
		required: true,
		default: 'binaryData',
		description: 'Choose how to provide the order document to process',
		displayOptions: {
			show: {
		...
		options: [
			{
				name: 'Binary Data',
				value: 'binaryData',
				description: 'Use order file from previous node',
			},
			{
				name: 'Base64 String',
				value: 'base64',
				description: 'Provide order content as base64 encoded string',
			},
			{
				name: 'URL',
				value: 'url',
				description: 'Provide URL to order file',
			},
		],
	},
	{
		...
		required: true,
		default: 'data',
		description: 'Name of the binary property that contains the order file (usually "data" for file uploads)',
		placeholder: 'data',
		displayOptions: {
			show: {
				operation: [ActionConstants.ProcessOrder],
	...
	},
	{
		...
		required: true,
		default: '',
		description: 'Base64 encoded order document content',
	...
	},
	{
		...
		required: true,
		default: '',
		description: 'URL to the order document file to process',
		placeholder: 'https://example.com/processorder.pdf',
		displayOptions: {
			show: {
				operation: [ActionConstants.ProcessOrder],
				inputDataType: ['url'],
	...
	},
	{
		...
		type: 'string',
		required: true,
		default: 'processorder.pdf',
		description: 'Name of the order document file for reference',
		placeholder: 'processorder.pdf',
		displayOptions: {
			show: {
				operation: [ActionConstants.ProcessOrder],
			},
...
/**
 * Process Order - Process order documents using PDF4ME API
 * Process: Read Order → Encode to base64 or upload blob → Send API request → Poll for completion → Return processed order data
 *
 * This action processes order documents:
 * - Returns structured JSON data with processed order information
...
export async function execute(this: IExecuteFunctions, index: number) {
	...
	if (!docContent || docContent.trim() === '') {
		throw new Error('Order document content is required');
	}
	...
	// Handle the response (processed order data)
	if (responseData) {
	...
	throw new Error('No order processing results received from PDF4ME API');
}

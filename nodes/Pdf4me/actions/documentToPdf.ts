...
export async function execute(this: IExecuteFunctions, index: number) {
	...
	if (inputDataType === 'binaryData') {
		// Get Word document content from binary data
		...
		// Use document URL directly - download the file first
		...
		console.log(`Downloading Word document from URL: ${documentUrl}`);
		docContent = await downloadDocumentFromUrl(documentUrl);
		...
		try {
			...
			// Validate the Word document content
			if (docContent.length < 100) {
				throw new Error('Word document appears to be too small. Please ensure the file is a valid Word document (.doc or .docx).');
			}
	...
	if (responseData) {
		...
		if (!fileName || fileName.trim() === '') {
			...
			const baseName = originalFileName ? originalFileName.replace(/\.[^.]*$/, '') : 'document_to_pdf_output';
...
async function downloadDocumentFromUrl(documentUrl: string): Promise<string> {
	/**
	 * Download Word document from URL and convert to base64
	 * Process: Download file → Convert to base64 → Validate content
	 * 
	 * Args:
	 *   documentUrl (str): URL to the Word document
	 * 
	 * Returns:
	 *   str: Base64 encoded Word document content
	 * 
	 * Raises:
	...
	try {
		...
		if (base64Content.length < 100) {
			throw new Error('Downloaded file appears to be too small. Please ensure the URL points to a valid Word document (.doc or .docx).');
		}

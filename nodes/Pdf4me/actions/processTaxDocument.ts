...
export const description: INodeProperties[] = [
	{
		...
		required: true,
		default: 'binaryData',
		description: 'Choose how to provide the tax document to process',
		displayOptions: {
			show: {
				operation: [ActionConstants.ProcessTaxDocument],
			},
		},
		options: [
			{
	...
	},
	{
		...
		required: true,
		default: 'data',
		description: 'Name of the binary property that contains the tax document file (usually "data" for file uploads)',
		placeholder: 'data',
		displayOptions: {
			show: {
				operation: [ActionConstants.ProcessTaxDocument],
	...
	},
	{
		...
		required: true,
		default: '',
		description: 'Base64 encoded tax document content',
	...
	},
	{
		...
		required: true,
		default: '',
		description: 'URL to the tax document file to process',
		placeholder: 'https://example.com/tax_document.pdf',
		displayOptions: {
			show: {
				operation: [ActionConstants.ProcessTaxDocument],
				inputDataType: ['url'],
	...
	},
	{
		...
		type: 'string',
		required: true,
		default: 'tax_document.pdf',
		description: 'Name of the tax document file for reference',
		placeholder: 'tax_document.pdf',
		displayOptions: {
			show: {
				operation: [ActionConstants.ProcessTaxDocument],
			},
		},
		hint: 'AI - Tax Document Data Extraction. See our <b><a href="https://docs.pdf4me.com/integration/n8n/pdf4me-ai/ai-process-tax-document/" target="_blank">complete guide</a></b> for detailed instructions and examples.',
	},
	{
		displayName: 'Tax Model',
		name: 'taxModel',
		type: 'options',
		default: '',
		description: 'Optional tax model for improved parsing accuracy',
		displayOptions: {
			show: {
				operation: [ActionConstants.ProcessTaxDocument],
			},
...
/**
 * Process Tax Document - Process tax documents using PDF4ME API
 * Process: Read Tax Document → Encode to base64 or upload blob → Send API request → Poll for completion → Return processed tax document data
 *
 * This action processes tax documents:
 * - Returns structured JSON data with processed tax document information
 * - Supports various tax models for improved parsing accuracy (W2, 1099, 1040, etc.)
...
export async function execute(this: IExecuteFunctions, index: number) {
	...
	const taxModel = this.getNodeParameter('taxModel', index) as string;
	...
	if (!docContent || docContent.trim() === '') {
		throw new Error('Tax document content is required');
	}
	...
	// Add optional taxModel if provided
	if (taxModel && taxModel.trim() !== '') {
		body.taxModel = taxModel.trim();
	}

...
export const description: INodeProperties[] = [
	{
		...
		displayOptions: {
			show: {
				operation: [ActionConstants.PptxToPdf],
			},
		},
		options: [
			{
...
export async function execute(this: IExecuteFunctions, index: number) {
	...
	if (operation === ActionConstants.ConvertToPdf) {
		...
		inputDataType = this.getNodeParameter('pptxInputDataType', index) as string;
		outputFileName = this.getNodeParameter('pptxOutputFileName', index) as string;
		docName = this.getNodeParameter('pptxDocName', index) as string;
		advancedOptions = this.getNodeParameter('pptxAdvancedOptions', index) as IDataObject;
	} else {
	...
	if (inputDataType === 'binaryData') {
		// Get PowerPoint content from binary data
		const binaryPropertyName = operation === ActionConstants.ConvertToPdf 
			? this.getNodeParameter('pptxBinaryPropertyName', index) as string
		...
		docContent = operation === ActionConstants.ConvertToPdf
			? this.getNodeParameter('pptxBase64Content', index) as string
		...
		// Use PowerPoint URL directly - download the file first
		const powerpointUrl = operation === ActionConstants.ConvertToPdf
			? this.getNodeParameter('powerpointUrl', index) as string
			: this.getNodeParameter('powerpointUrl', index) as string;
		
		// Validate URL format
		try {
			new URL(powerpointUrl);
		} catch (error) {
			throw new Error('Invalid URL format. Please provide a valid URL to the PowerPoint file.');
		}

		console.log(`Downloading PowerPoint from URL: ${powerpointUrl}`);
		docContent = await downloadPowerpointFromUrl(powerpointUrl);
		console.log('PowerPoint file successfully downloaded and encoded to base64');
	} else if (inputDataType === 'filePath') {
		// Use local file path - read the file and convert to base64
		const filePath = operation === ActionConstants.ConvertToPdf
			? this.getNodeParameter('pptxFilePath', index) as string
			: this.getNodeParameter('filePath', index) as string;

		// Validate file path (basic check)
		if (!filePath.includes('/') && !filePath.includes('\\')) {
			throw new Error('Invalid file path. Please provide a complete path to the PowerPoint file.');
		}
		...
		try {
			...
			// Validate the PowerPoint content
			if (docContent.length < 100) {
				throw new Error('PowerPoint file appears to be too small. Please ensure the file is a valid PowerPoint presentation.');
			}
	...
	if (!docContent || docContent.trim() === '') {
		throw new Error('PowerPoint content is required');
	}
	...
	if (responseData) {
		...
		if (!fileName || fileName.trim() === '') {
			...
			const baseName = originalFileName ? originalFileName.replace(/\.[^.]*$/, '') : 'presentation_to_pdf_output';
...
async function downloadPowerpointFromUrl(powerpointUrl: string): Promise<string> {
	/**
	 * Download PowerPoint from URL and convert to base64
	 * Process: Download file → Convert to base64 → Validate content
	 * 
	 * Args:
	 *   powerpointUrl (str): URL to the PowerPoint file
	 * 
	 * Returns:
	 *   str: Base64 encoded PowerPoint content
	 * 
	 * Raises:
	...
	try {
		...
		const response = await fetch(powerpointUrl);
		
		if (!response.ok) {
			throw new Error(`Failed to download PowerPoint from URL: ${response.status} ${response.statusText}`);
		}
		...
		if (base64Content.length < 100) {
			throw new Error('Downloaded file appears to be too small. Please ensure the URL points to a valid PowerPoint file.');
		}

		console.log(`PowerPoint downloaded successfully: ${buffer.length} bytes`);
		return base64Content;
		
	} catch (error) {
		if (error.message.includes('Failed to fetch')) {
			throw new Error(`Network error downloading PowerPoint from URL: ${error.message}`);
		} else if (error.message.includes('Failed to download')) {
			throw new Error(`HTTP error downloading PowerPoint: ${error.message}`);
		} else {
			throw new Error(`Error downloading PowerPoint from URL: ${error.message}`);
		}
	}
} 
...
export async function execute(this: IExecuteFunctions, index: number) {
	const inputDataType = this.getNodeParameter('pdfToPowerpointInputDataType', index) as string;
	const outputFileName = this.getNodeParameter('pdfToPowerpointOutputFileName', index) as string;
	const docName = this.getNodeParameter('pdfToPowerpointDocName', index) as string;
	const advancedOptions = this.getNodeParameter('pdfToPowerpointAdvancedOptions', index) as IDataObject;

	let docContent: string;
	let originalFileName = docName;

	if (inputDataType === 'binaryData') {
		const binaryPropertyName = this.getNodeParameter('pdfToPowerpointBinaryPropertyName', index) as string;
		...
		docContent = this.getNodeParameter('pdfToPowerpointBase64Content', index) as string;
		if (docContent.includes(',')) {
			docContent = docContent.split(',')[1];
		}
	} else if (inputDataType === 'url') {
		const pdfUrl = this.getNodeParameter('pdfToPowerpointUrl', index) as string;
		try {
			new URL(pdfUrl);
		} catch (error) {
		...
		const filePath = this.getNodeParameter('pdfToPowerpointFilePath', index) as string;
	...
	if (responseData) {
		...
		if (!fileName || fileName.trim() === '') {
			const baseName = originalFileName ? originalFileName.replace(/\.[^.]*$/, '') : 'pdf_to_powerpoint_output';
		...
		if (Buffer.isBuffer(responseData)) {
			pptxBuffer = responseData;
		} else if (typeof responseData === 'string') {
			pptxBuffer = Buffer.from(responseData, 'base64');
		} else {
			pptxBuffer = Buffer.from(responseData as any);
		}
		if (!pptxBuffer || pptxBuffer.length < 100) {
			throw new Error('Invalid PowerPoint response from API. The converted file appears to be too small or corrupted.');
		}
